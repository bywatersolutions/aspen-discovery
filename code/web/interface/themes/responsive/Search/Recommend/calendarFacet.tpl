<form id='{$title}Filter' action='{$fullPath}' class="form-horizontal-narrow">
	<div class="facet-form">
		<div class="form-group">
			...
			<input type="date" name="{$title}Start" id="{$title}Start" placeholder="mm/dd/yyyy" class="form-control" size="10" {if empty($cluster.allowPastDates)}min="{$smarty.now|date_format:"%Y-%m-%d"}"{/if} {if !empty($maxEventDate)}max="{$maxEventDate|date_format:"%Y-%m-%d"}"{/if} {if !empty($cluster.start)}value="{$cluster.start}"{/if}>
		</div>
		<div class="form-group">
			<label class="control-label" for="{$title}End">{translate text="To" isPublicFacing=true}</label>
			<input type="date" name="{$title}End" id="{$title}End" placeholder="mm/dd/yyyy" class="form-control" size="10" {if empty($cluster.allowPastDates)}min="{$smarty.now|date_format:"%Y-%m-%d"}"{/if} {if !empty($maxEventDate)}max="{$maxEventDate|date_format:"%Y-%m-%d"}"{/if} {if !empty($cluster.end)}value="{$cluster.end}"{/if}>
		</div>
