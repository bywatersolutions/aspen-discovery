			...
			<div id="selected-browse-label">
				<div class="btn-toolbar" role="toolbar" aria-label="Search Tools" style="justify-content: flex-end">
					...
					<div class="btn-group btn-group-sm" data-toggle="buttons" style="margin-right: 1rem">
						<label for="covers" title="Covers" class="btn btn-sm btn-default">
							<input onchange="AspenDiscovery.Searches.toggleDisplayMode(this.id)" type="radio" id="covers">
							<i class='fas fa-th'></i> {translate text=Covers isPublicFacing=true}
						</label>
						<label for="list" title="Lists" class="btn btn-sm btn-default">
							<input onchange="AspenDiscovery.Searches.toggleDisplayMode(this.id)" type="radio" id="list">
							<i class='fas fa-list'></i> {translate text=List isPublicFacing=true}
						</label>
					</div>
					{if $showSearchTools || ($loggedIn && count($userPermissions) > 0)}
						<div class="btn-group btn-group-sm">
							<button data-toggle="dropdown" class="btn btn-sm btn-default dropdown-toggle" type="button" id="dropdownSearchToolsBtn">
								<i class="fas fa-toolbox"></i> {translate text='Search Tools' isPublicFacing=true} <span class="caret"></span>
							</button>
							<ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownSearchToolsBtn">
								{if !empty($showSearchTools)}
									...
									{if !empty($risLink)}
										<li>
											<a href="{$risLink|escape}">{translate text="Export To RIS" isPublicFacing=true}</a>
										</li>
									{/if}
								{/if}
